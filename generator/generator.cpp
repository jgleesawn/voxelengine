...
Mesh * Generator::generate(const glm::vec4 & llb, const float & size, densityFunc df) {
	voxelpolymacro
	...
	for( int k=0; k<32; k++ ) {
		...
		for( int l=0; l<voxelpoly.numPolys[ind]; l++ ) {
			for( int m=0; m<3; m++ ) {
				...
				int x = (vboind>>5) %32;
				int y = (vboind>>10) %32;	
				mesh->vbo[vboind] = ((float)(fabs(d1)/(fabs(d1)+fabs(d2)))*v)+glm::vec4((float)(j+x)*ss, (float)(k+y)*ss, (float)(i+z)*ss, 0.0f);
				mesh->ibo[ibopos++] = vboind;
			}
		}
	}
	}
	}
	mesh->indexCount = ibopos;
	return mesh;
}
